...
window.addEventListener("click", function (e) {
  if (
    ...
    e.target.className == "span-url" &&
    e.target.attributes.href.value != undefined
  ) {
...
function getDataFromStorage() {
  if (
    tabsFromBackground != undefined &&
    tabsFromBackground != null &&
    tabsFromBackground.length > 0
  )
...
function getLimitsListFromStorage(items) {
  if (items !== undefined) restrictionList = items;
...
function getTabsFromStorage(tabs) {
  ...
  for (var i = 0; i < targetTabs.length; i++) {
    ...
    var counter;
    if (currentTypeOfList === TypeListEnum.ToDay) {
      summaryTime = targetTabs[i].days.find(
        (x) => x.date == todayLocalDate()
      ).summary;
      let item = targetTabs[i].days.find((x) => x.date == todayLocalDate());
      if (item != null) counter = item.counter;
    }
    if (currentTypeOfList === TypeListEnum.All) {
  ...
  if (currentTypeOfList === TypeListEnum.ToDay) ui.drawChart(tabsForChart);
...
function getTabsFromStorageForExpander(tabs) {
  ...
  for (var i = 31; i < targetTabs.length; i++) {
    ...
    if (currentTypeOfList === TypeListEnum.ToDay) {
      ...
      let item = targetTabs[i].days.find((x) => x.date == todayLocalDate());
      if (item != undefined) counter = item.counter;
    }
    if (currentTypeOfList === TypeListEnum.All) {
      summaryTime = targetTabs[i].summaryTime;
      counter = targetTabs[i].counter;
...
function getTotalTime(tabs) {
  ...
  if (currentTypeOfList === TypeListEnum.ToDay) {
    var summaryTimeList = tabs.map(function (a) {
      return a.days.find((s) => s.date === todayLocalDate()).summary;
    });
...
function getFirstDay() {
  ...
  tabsFromStorage.map(function (a) {
    return a.days.map(function (a) {
      if (array.indexOf(a.date) === -1) return array.push(a.date);
    });
  });
...
function setStatData(array) {
  ...
  arrayAscByTimeWithoutCurrentDay = arrayAscByTimeWithoutCurrentDay.sort(
    function (a, b) {
      return a.total - b.total;
    }
  );
  ...
  stat.activeDay = new Date(
    arrayAscByTime[arrayAscByTime.length - 1].date
  ).toLocaleDateString();
  ...
  if (arrayAscByTimeWithoutCurrentDay.length > 0) {
    stat.inActiveDayWithoutCurrentDay = new Date(
      arrayAscByTimeWithoutCurrentDay[0].date
    ).toLocaleDateString();
    stat.activeDayWithoutCurrentDay = new Date(
      arrayAscByTimeWithoutCurrentDay[
        arrayAscByTimeWithoutCurrentDay.length - 1
      ].date
    ).toLocaleDateString();
    stat.inActiveDayTimeWithoutCurrentDay =
      arrayAscByTimeWithoutCurrentDay[0].total;
    stat.activeDayTimeWithoutCurrentDay =
      arrayAscByTimeWithoutCurrentDay[
        arrayAscByTimeWithoutCurrentDay.length - 1
      ].total;
  } else {
...
function getTabsByDays(tabs) {
  ...
  if (
    range.from !== "Invalid Date" &&
    range.to !== "Invalid Date" &&
    isCorrectDate(range)
  ) {
    var listOfDays = [];
    tabs.forEach((tab) => {
...
function getTabsFromStorageByDay(day, blockName) {
  ...
  targetTabs = tabsFromStorage.filter((x) =>
    x.days.find((s) => s.date === day)
  );
  if (targetTabs.length > 0) {
    targetTabs = targetTabs.sort(function (a, b) {
      return (
        b.days.find((s) => s.date === day).summary -
        a.days.find((s) => s.date === day).summary
      );
    });
...
function fillBlockWithInActiveDay() {
  ...
  if (flag == "true")
    fillValuesForBlockWithInActiveDay(
      "statInActiveDay",
      stat.inActiveDayWithoutCurrentDay,
      stat.inActiveDayTimeWithoutCurrentDay,
      flag
    );
  else
...
function fillValuesForBlockWithInActiveDay(prefix, dayValue, timeValue, flag) {
  if (flag == "true") {
    ...
    document
      .getElementById(prefix + "WithoutCurrentDay")
      .classList.remove("hide");
    document
      .getElementById(prefix + "TimeWithoutCurrentDay")
      .classList.remove("hide");

    document.getElementById(prefix + "Title").innerHTML =
      "Include the current day in the calculation of statistics";
