...
class Activity {
  ...
  isLimitExceeded(domain, tab) {
    if (
      ...
      var item = setting_restriction_list.find((o) =>
        isDomainEquals(extractHostname(o.domain), extractHostname(domain))
      );
      if (item !== undefined) {
  ...
  wasDeferred(domain) {
    if (deferredRestrictionsList != undefined) {
      let defItem = deferredRestrictionsList.find(
      ...
      if (defItem != null) {
        ...
        if (time + DEFERRED_TIMEOUT > new Date().getTime()) {
          ...
          if (index > -1) deferredRestrictionsList.splice(index, 1);
  ...
  addTimeInterval(domain) {
    var item = timeIntervalList.find(
      (o) => o.domain === domain && o.day == todayLocalDate()
    );
    if (item != undefined) {
  ...
  closeIntervalForCurrentTab() {
    if (currentTab !== "" && timeIntervalList != undefined) {
      var item = timeIntervalList.find(
        (o) => o.domain === currentTab && o.day == todayLocalDate()
      );
